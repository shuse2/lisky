...
export function itShouldThrowValidationError() {
	...
	testFunction.should.throw().and.has.property('name').which.equal('ValidationError');
	return testFunction.should.throw().and.has.property('message').which.include(message);
}
...
export function itShouldThrowFileSystemError() {
	...
	const message = getFirstQuotedString(this.test.title);
	testFunction.should.throw().and.is.instanceOf(Error);
	testFunction.should.throw().and.has.property('name').which.equal('FileSystemError');
	return testFunction.should.throw().and.has.property('message').which.include(message);
}
